...
IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260813112913_AddOutboxRetryColumns')
BEGIN
    DROP INDEX ix_outbox_messages_processed_at_utc ON outbox_messages;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260813112913_AddOutboxRetryColumns')
BEGIN
    ALTER TABLE `outbox_messages` ADD `attempt_count` int NOT NULL DEFAULT 0;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260813112913_AddOutboxRetryColumns')
BEGIN
    ALTER TABLE `outbox_messages` ADD `next_attempt_at_utc` datetime(6) NULL;
END;
...
BEGIN
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260813112913_AddOutboxRetryColumns', '10.0.10');
END;
